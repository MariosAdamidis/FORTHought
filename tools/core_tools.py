"""
title: FORTHought Core Tools
description: Universal document and presentation gateway.
    Document creation (DOCX, PDF, XLSX, CSV, PPTX, HTML, TXT, MD),
    document reading and editing, and PPTX presentation generation.
    Every model gets this tool — it handles all file operations.
...
log = logging.getLogger("forthought.tools.core")
...
# ═══════════════════════════════════════════════════════════════════════════
#  Registry — Core profile: files, presenton
...
class Tools:
    class Valves(BaseModel):
        files_url: str = Field(
            default="http://localhost:9004/mcp", description="Files MCP server"
        )
        presenton_url: str = Field(
            default="http://localhost:9013/mcp",
            description="Presenton PPTX MCP server",
        )
        files_admin_api_key: str = Field(
            default="", description="Fallback OWUI API key (sk-...) for files server"
        )
    ...
    async def run(
        ...
        **kwargs,
        ...
        :param tool: Tool name (e.g. "doc.create", "doc.read", "pptx.generate"). Use find() to list them.
    ...
    async def find(
        ...
        **kwargs,
        ...
        :param query: Filter by keyword (e.g. "docx", "pptx", "edit"). Leave empty for all.
        :param server: Filter by server -- files, pptx. Leave empty for all.
        """
    ...
    async def create_document(
        ...
        **kwargs,
        ...
        result = await _call_mcp(
            ...
            _mint_user_jwt(__user__) or self.valves.files_admin_api_key or None,
        )
