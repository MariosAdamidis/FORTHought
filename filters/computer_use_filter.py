...
author_url: https://github.com/Yambr/open-computer-use
version: 3.3.0
...
CHANGELOG (v3.3.0):
- S193 FIX: iframe srcdoc height collapse. OWUI renders HTML code block
  artifacts via iframe srcdoc, not src. When our iframe was the only child,
  the browser-synthesized <html>/<body> wrappers defaulted to auto-height,
  collapsing our height:100% to zero. Inner preview SPA .preview flex
  container got no space -> sine_wave.png rendered at natural 640x480
  inside a cramped container, Browser tab canvas had 0x0 dimensions.
- Fix: outlet() now emits <style>html,body{height:100%;margin:0}
  iframe{width:100%;height:100%;...}</style> inline with the <iframe>,
  giving the srcdoc's implicit html/body explicit height. Inner iframe
  fills the artifact panel -> preview SPA has real viewport -> image
  scales properly, xterm canvas renders, Browser CDP viewer works.
- Idempotency check changed from full-block string to preview_url +
  iframe substring match (style prefix would otherwise break dedup).
...
class Filter:
    class Valves(BaseModel):
        ...
        ENABLE_PREVIEW_ARTIFACT: bool = Field(
            default=False,
            description="Append an inline <iframe> artifact rendering /preview/{chat_id} to assistant messages with file links. Disabled by default — preview_url_detection frontend patch handles sidebar auto-show without needing a code block.",
        )
    ...
    def outlet(
        ...
        for message in body.get("messages", []):
            ...
            if self.valves.ENABLE_PREVIEW_ARTIFACT:
                # S193: OWUI renders HTML artifacts via iframe srcdoc. When our
                # iframe is the only child, the browser-synthesized <html>/<body>
                # default to auto-height, which collapses our height:100%.
                # Inline a <style> so the srcdoc's implicit html/body get real
                # height, letting the inner iframe fill the artifact panel.
                iframe = (
                ...
                # Idempotency: check on the src URL, not the full block (style
                # prefix could otherwise cause duplicates if outlet re-runs).
                if preview_url not in content or '<iframe' not in content:
                    content += "\n\n```html\n" + iframe + "\n```"
